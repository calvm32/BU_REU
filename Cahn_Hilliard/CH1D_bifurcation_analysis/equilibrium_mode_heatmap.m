...
epsilon = 0.025; % type == 1
epsilon_list = linspace(0, 0.1, 50); % type == 2 or 3
...
switch type
    ...
    case 2
        ...
        % parfor requires a cell array to accumulate results (no indexed slices of 3D arrays)
        num_eps = length(epsilon_list);
        num_IC = length(IC_modes);
        Heat_cell = cell(num_eps, 1);  % one 2D slice per worker
        ...
        parfor eidx = 1:num_eps
            ...
            mu = @(t_val) epsilon * t_val;

            Heat_local = zeros(num_IC, num_IC);  % local accumulator for this epsilon
        ...
        %% Reassemble 3D Heat array from cell array
        ...
        v.FrameRate = 10*length(epsilon_list);
        open(v);
        ...
        for eidx = 1:num_eps
            ...
            text(0.5, -0.1, sprintf('Parameters: T = %.0f,  t0 = %.0f, \\epsilon = %.3f\nEach IC ran for %.0f trials', ...
                T, t0, epsilon_list(eidx), num_runs), ...
