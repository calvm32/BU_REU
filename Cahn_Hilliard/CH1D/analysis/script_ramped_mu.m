...
mass = (2*n_subc+1)*sqrt(2)/(2*L); 

% if starting with a single dominant mode:
k_init = 1;
...
mu0 = critical_bifurcation_eig(1, L, mass) - 0.2; %(mu_n + mu_next)/2;
...
T_k = @(k, mu0, mass, ep) ( -lin_term(k,mu0,mass) + sqrt( lin_term(k,mu0,mass)^2 + 2*log(10)*ep*k_hat_sq(k) ))/(ep*k_hat_sq(k)); % time at which IC mode, when dominant, starts growing past starting value

disp(sprintf('starting mode = %.0d', k_init))
disp(sprintf('lambda(k)     = %.4d', lambda_k(k_init, mu0, mass)))
disp(sprintf('freeze-out    = %.4f', T_k(k_init, mu0, mass, epsilon)))
...
switch type
    case 0
        ...
        %muf = critical_bifurcation_eig(20, L, mass);
        %T = (muf - mu0)/ep;
        T = T_k(k_init, mu0, mass, ep) + 4000;
        mu = @(t) mu0 + t * ep;
...
function mu = critical_bifurcation_eig(k, L, mass)
    % Computes the j'th frequencies critical mu and associated eigenvalue
    mu =  3*(mass)^2 + (k/L)^2 ;    
end

function [MUS, KS] = critical_bifurcation_inteig(j, L, mass, mu0)
    % Computes the j'th frequencies critical mu and associated eigenvalue
    KS = 2 * pi * j./ L;
